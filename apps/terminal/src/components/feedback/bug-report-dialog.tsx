...
/**
 * "Report a bug" — the one place a user can talk back from inside the app.
 *
 * Delivery rides the existing opt-in PostHog pipe as a plain `bug_report`
 * event rather than a PostHog Survey: surveys have to be created and matched
 * server-side, so a build pointed at a project without that survey would
 * silently have no feedback channel at all. An event we declare ourselves
 * always lands.
 *
...
const ISSUES_URL = 'https://github.com/Pairlens/trading-terminal/issues'
...
export function BugReportDialog({ open, onOpenChange }: BugReportDialogProps) {
  ...
  const close = () => {
    onOpenChange(false)
    // Reset after the close animation so the form doesn't blank mid-fade.
    window.setTimeout(() => {
      setMessage('')
      setCategory('bug')
    }, 200)
          ...
          <div className="space-y-2">
            ...
            >
              <SelectTrigger className="w-40" id="bug-report-category">
                <SelectValue>
                  {(value: string) => categoryLabel(value)}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
