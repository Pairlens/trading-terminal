...
export function CommitBar({ hasCycles, onBeforeCommit }: CommitBarProps) {
  ...
  const changeCount = useNotificationStore(
    (s) => s.draft?.pendingChanges.length ?? 0,
  )
  const commitDraft = useNotificationStore((s) => s.commitDraft)
  const discardDraft = useNotificationStore((s) => s.discardDraft)
  const getPendingDiff = useNotificationStore((s) => s.getPendingDiff)

  const [diffOpen, setDiffOpen] = useState(false)

  // Full structural + per-step validation, recomputed as the draft changes.
  // Cycles are surfaced separately by the canvas (red edges).
