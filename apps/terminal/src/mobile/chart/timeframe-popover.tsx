 ...
 *
 * The interval list comes from the desktop chart toolbar
 * (`supportedTimeframeOptions`, exported for this file) rather than a second
 * copy: a build that ships a new interval must not ship it to one surface
 * only, and neither surface may offer an interval the charted venue does not
 * serve. The stored value stays lowercase (`1d`); `short` is the label the
 * design draws (`1D`).
 *
...
export default memo(function TimeframePopoverChip() {
  ...
  const { timeframe, supportedTimeframes } = useChartConfig()
  const { setTimeframe } = useChartActions()
  const { pinned, touch, promote } = usePinnedTimeframes()
  const [open, setOpen] = useState(false)

  // Only what the venue being charted serves. The pinned row is a preference
  // over the whole list, so on a venue with four intervals it thins out rather
  // than offering a chip whose chart would never seed — and the pins come back
  // untouched on the next CEX.
    ...
    <>
      ...
      {open ? (
        ...
        <Portal>
          ...
          >
            {/* A venue with three intervals can leave either grid empty, and
                a header over no cells reads as a failed load. Each section is
                drawn only when it has something in it. */}
            ...
            {moreOptions.length > 0 ? (
              <>
                <p
                  className={cn(
                    'px-1 pb-2 text-[9.5px] font-semibold uppercase tracking-[.09em] text-muted-foreground',
                    pinnedOptions.length > 0 ? 'pt-[14px]' : 'pt-[3px]',
                  )}
                >
