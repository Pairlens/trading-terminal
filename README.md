...
The kind of trading desk that usually costs thousands of dollars a year: free, private, and entirely yours. 14 crypto spot exchanges, perpetual futures, US equities, prediction markets, NFT collections, on-chain DEX trading and a launchpad board for memecoins in one desk, with professional charts, an AI assistant that can drive all of it, and your keys on your machine.
...
- **Every market on one desk.** Trade spot on OKX, Binance, Coinbase, Kraken, and 10 more exchanges, US equities through Alpaca, event contracts on Kalshi and Polymarket, NFT collections on OpenSea, and on-chain DEXs across Solana, Ethereum, Base, Arbitrum, BNB Chain, and Polygon, side by side in the same interface.
...
- **One AI assistant for the whole terminal.** It sits in a dock in the corner, follows you from the chart to the workbench to the bots page in one conversation, and it sees what you are looking at. 113 tools: read any market, drive the chart, run sourced research, write Python indicators and strategies, backtest them, deploy bots, build workflows and alerts. Trades it prepares are proposals you explicitly confirm, and it never overrides your risk guardrails, which are enforced below the AI, not by it. Bring your own AI key (Groq, OpenAI, DeepSeek, Anthropic, OpenRouter) or subscribe to the hosted Pairlens Intelligence plan.
...
| NFT marketplaces   | OpenSea (reads on 6 chains, orders signed on Ethereum and Base; bring your own free key)                          |
| DEX trading        | Jupiter (Solana), EVM DEX connector (Ethereum, Base, Arbitrum, BSC, Polygon)                                      |
| DEX market data    | GeckoTerminal, DexPaprika                                                                                         |
| NFT market data    | CoinGecko NFT (keyless fallback: floor, volume, supply, holders)                                                  |
| AI inference       | Groq, OpenAI, DeepSeek, Anthropic, OpenRouter (bring your own key)                                                |
