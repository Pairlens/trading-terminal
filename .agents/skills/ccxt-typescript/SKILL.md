...
> deep-import `ccxt/js/src/pro/<id>.js` through
> `CcxtVenueConfig.loadExchangeClass`. Instances are constructed by
> `ccxt-connector/exchange-host.ts` (transport, liveness, sandbox and geo
> routing), not ad hoc. Read the "CCXT bridge" section in CLAUDE.md before
