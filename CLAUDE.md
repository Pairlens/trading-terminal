...
- **AI inference** (bring-your-own-key): Groq, OpenAI, DeepSeek, Anthropic, OpenRouter
...
- **Conversations are many, local-first, and synced only on request.** Threads live in `stores/assistant-conversations-store.ts`: a small index key plus one key per thread, holding whole `UIMessage`s so tool parts, research cards and order proposals survive a reload. Every thread keeps its own AI SDK `Chat` in `lib/assistant-core/chat-sessions.ts` (`AssistantChatSessions`), and the hook is handed the instance (`useChat({ chat })`), so clicking another History row only changes which Chat is on screen: the previous run keeps streaming and persisting in the background, the orb reports the busiest live run, and deleting a conversation is the one thing that aborts its run (`prune`). Titles come from `lib/assistant-core/conversation-title.ts`: the first message names the row instantly, the model is asked in the background for a better one.
