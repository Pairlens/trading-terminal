...
- **Core**: `pairlens-core` (instrument discovery, panels, workflow step types), `pairlens-intelligence` (fallback-only AI inference/search + discovery + symbol logos), `pairlens-predictions` (zero capabilities: the `events` browser and the `prediction-positions` pane, plus the prediction desk and the `template:prediction-discovery` home board, kept out of pairlens-core so a deployment that drops the `predictions` family drops them too), `pairlens-cex-futures` (same shape: the `futures-positions` pane plus the perps desk), `pairlens-dex` and `pairlens-equities` (presets only, no panels at all: every pane they arrange already ships in pairlens-core, so what belongs to the family is the arrangement). `basic-symbols` is deprecated (absorbed into pairlens-core, kept for registry back-compat).
