...
| Assistant conversations            | localStorage on that device (`stores/assistant-conversations-store.ts`) | Privacy: transcripts are never uploaded |
...
- **Conversations are many, and local.** Threads live in `stores/assistant-conversations-store.ts`: a small index key plus one key per thread, holding whole `UIMessage`s so tool parts, research cards and order proposals survive a reload. There is NO server copy and no sync domain for chat any more (`api.getAiMessages`/`saveAiMessage`/`clearAiMessages`, the `copilot` sync domain and the persistence adapter's `getAIMessages`/`appendAIMessage` are all gone): a transcript must never reach the App Server. The chat is keyed on the active thread id, and `AssistantConversationInner` holds its own `threadId` so a switch can stop and flush the outgoing run BEFORE the SDK rebuilds its `Chat` around another thread. Titles come from `lib/assistant-core/conversation-title.ts`: the first message names the row instantly, the model is asked in the background for a better one.
