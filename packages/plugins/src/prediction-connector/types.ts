...
export type PredictionVenueConfig = {
   ...
   *
   * Declared rather than discovered, for the same reason `defaultEventScope`
   * is: each family is a venue-native SERIES, series are not searchable, and
   * the connector is the only thing that knows which of its own series are the
   * up/down conveyor. Absent means the venue lists none, and the
   * `crypto-updown` preset answers with an empty list rather than an error.
   */
